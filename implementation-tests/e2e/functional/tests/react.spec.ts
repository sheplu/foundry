...
test.describe('React canary — reference screen', () => {
  ...
  test('re-clicking the currently-pressed button is a no-op', async ({ page }) => {
    // Ensure we start from grid (may follow the kanban click above in parallel
    // test runs; make the test independent by re-asserting).
  ...
  test('typing into the search input filters the visible options', async ({ page }) => {
    ...
    // United Kingdom + United States match "un"; others should be hidden.
