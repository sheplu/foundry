...
- **Pagination** (`data-testid="pagination-row"`)
  - One `<foundry-pagination data-testid="pagination-main" page="3" total="12">`, demonstrating the middle-range truncation (ellipsis on both sides) at a non-boundary page. The canary wires a `change` listener that mirrors `event.detail.page` back onto the pager's `page` attribute (controlled API) and writes the new page into a sibling `<pre data-testid="pagination-result">`.
  - Expects: initial rendering shows `prev`, `1`, `…`, `2`, `3`, `4`, `…`, `12`, `next`. The current page (`3`) carries `aria-current="page"` and the `page-current` part. Clicking `4` fires `change` → the canary writes `4` to `pagination-result` AND sets the pager to `page="4"` (the rendered buttons update accordingly). Clicking prev decrements; prev is `disabled` at `page=1`. Each button carries an aria-label (`Go to page 4`, `Previous`, `Next`, `More pages`).
