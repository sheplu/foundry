...
- **Combobox** (`data-testid="combobox-row"`)
  - One `<foundry-combobox data-testid="cb-city" name="city" placeholder="Type a city">` with a `<span slot="label">City</span>`, a `<span slot="helper">Suggestions; you can also type your own.</span>`, and five `<foundry-option>` children: `paris` (Paris), `london` (London), `tokyo` (Tokyo), `new-york` (New York), `sydney` (Sydney). Demonstrates the free-text-with-suggestions flow: the user can pick a suggestion or commit whatever they type. The canary wires a `change` listener that writes `event.detail.value` into a sibling `<pre data-testid="combobox-result">`.
  - Expects: focusing the inner input and pressing `ArrowDown` opens the listbox (`[open]` on the host, `aria-expanded="true"` on the inner input). Clicking the `Paris` option fills the inner input with `Paris`, fires `change` with `detail.value === 'paris'`, closes the listbox, and updates the `<pre data-testid="combobox-result">` to `paris`. Typing a non-option string (e.g., `Atlantis`) and pressing `Tab` commits that text — `change` fires with `detail.value === 'Atlantis'`, the listbox closes, and the result `<pre>` shows `Atlantis`.
- **Date picker** (`data-testid="date-picker-row"`)
  - One `<foundry-date-picker data-testid="dp-dob" name="dob" min="1900-01-01" max="2030-12-31">` with a `<span slot="label">Date of birth</span>` and a `<span slot="helper">Format: YYYY-MM-DD.</span>`. Demonstrates the editable-input + popover-calendar flow: users can type an ISO date OR open the popover and pick a day. The canary wires a `change` listener that writes `event.detail.value` into a sibling `<pre data-testid="date-picker-result">`.
  - Expects: focusing the inner input and pressing `ArrowDown` opens the popover (`[open]` on the host, `aria-expanded="true"` on the inner input, `role="dialog"` on the surface, `role="grid"` inside). Clicking a day cell fills the inner input with the ISO string, fires `change` with `detail.value` being that ISO string and `detail.source === 'pick'`, closes the popover, and updates the `<pre data-testid="date-picker-result">` accordingly. Typing a valid in-range ISO (e.g., `2026-05-19`) and pressing `Tab` commits the typed value — `change` fires with `detail.source === 'blur'`, and the result `<pre>` shows `2026-05-19`.
- **Number stepper** (`data-testid="number-stepper-row"`)
  - One `<foundry-number-stepper data-testid="ns-qty" name="quantity" min="0" max="100" step="1" value="1">` with a `<span slot="label">Quantity</span>` and a `<span slot="helper">Between 0 and 100.</span>`. Demonstrates the spinbutton-with-side-buttons flow: users can click `+`/`−`, type a number, or use ArrowUp/ArrowDown on the focused input. The canary wires a `change` listener that writes `event.detail.value` into a sibling `<pre data-testid="number-stepper-result">1</pre>`.
  - Expects: clicking the inner `+` button (`[part="increment"]`) increments the host's `value` by `step`, fires `change` with `detail.source === 'increment'`, and updates the result `<pre>` to `2`. Pressing `ArrowUp` on the inner input also increments by `step` (fires `change` with `detail.source === 'key'`). Typing `7` in the inner input and pressing `Tab` commits the typed value; the result `<pre>` shows `7`.
- **Field wrapper** (`data-testid="field-row"`)
  - Demonstrates `<foundry-field>` wrapping a non-foundry control. Renders two side-by-side examples:
    - `<foundry-field data-testid="field-birthday">` containing `<span slot="label">Birthday</span>`, a native `<input type="date" data-testid="field-birthday-input">`, and `<span slot="helper">Native date pickers vary by browser.</span>`. Expects the inner `<input>` to carry `aria-labelledby="<labelId>"` and `aria-describedby="<helperId>"` after mount.
    - `<foundry-field required invalid data-testid="field-website">` containing `<span slot="label">Website</span>`, a native `<input type="url" data-testid="field-website-input">`, and `<span slot="error">Please enter a valid URL.</span>`. Expects the host to render the `*` marker on the label and the inner `<input>` to carry `aria-invalid="true"`, `aria-errormessage="<errorId>"`, and `aria-describedby="<errorId>"`.
- **Table** (`data-testid="table-row"`)
