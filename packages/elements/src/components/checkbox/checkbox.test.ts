...
function uniqueSubclass(): { tag: string } {
  const tag = `foundry-checkbox-test-${++counter}`;
  class Sub extends FoundryCheckbox {}
...
function getInput(el: HTMLElement): HTMLInputElement {
  ...
  if (!(inp instanceof HTMLInputElement)) throw new Error('inner checkbox not found');
  return inp;
}
...
describe('FoundryCheckbox.define', () => {
  it('registers the given tag with FoundryCheckbox', () => {
    const tag = `foundry-checkbox-define-${++counter}`;
    FoundryCheckbox.define(tag);
    expect(customElements.get(tag)).toBe(FoundryCheckbox);
  });

  it('does not re-register an existing tag', () => {
    const tag = `foundry-checkbox-noop-${++counter}`;
    class Existing extends FoundryCheckbox {}
    customElements.define(tag, Existing);

    expect(() => FoundryCheckbox.define(tag)).not.toThrow();
    expect(customElements.get(tag)).toBe(Existing);
  });
});

describe('FoundryCheckbox defaults', () => {
  it('declares formAssociated for the custom element registry', () => {
    expect(FoundryCheckbox.formAssociated).toBe(true);
  });

  it('defaults checked to false and value to "on"', () => {
    const { tag } = uniqueSubclass();
    const el = document.createElement(tag) as FoundryCheckbox & { value: string; checked: boolean };
  ...
  it('defaults boolean flags to false', () => {
    const { tag } = uniqueSubclass();
    const el = document.createElement(tag) as FoundryCheckbox;
    document.body.appendChild(el);
...
describe('FoundryCheckbox attribute forwarding', () => {
  ...
  it('re-forwards when the host attribute changes at runtime', () => {
    const { tag } = uniqueSubclass();
    const el = document.createElement(tag) as FoundryCheckbox & { disabled: boolean };
    document.body.appendChild(el);
...
describe('FoundryCheckbox checked coupling', () => {
  ...
  it('writing the host checked property updates the inner input', () => {
    const { tag } = uniqueSubclass();
    const el = document.createElement(tag) as FoundryCheckbox & { checked: boolean };
    document.body.appendChild(el);
  ...
  it('writing checked to its current value is a no-op on the inner input', () => {
    ...
    // Setting to the same value should not rewrite the inner input (exercises
    // the `if (this.#input.checked !== c)` equality branch).
    el.checked = true;
    expect(getInput(el).checked).toBe(true);
  });
...
describe('FoundryCheckbox validity', () => {
...
describe('FoundryCheckbox form-associated lifecycle', () => {
...
describe('FoundryCheckbox accessors and focus', () => {
  ...
  it('reportValidity falls back to the inner input when internals lacks it', () => {
    ...
    el.setAttribute('required', '');
    document.body.appendChild(el);
    // jsdom's ElementInternals stub omits reportValidity, so the fallback runs.
...
describe('FoundryCheckbox with patched ElementInternals', () => {
  ...
  it('calls setFormValue(value) when checked', () => {
    withPatchedInternals((stub) => {
      ...
      el.checked = true;
      expect(stub.setFormValue).toHaveBeenCalledWith('weekly');
...
describe('FoundryCheckbox slot handling', () => {
  it('reflects has-label when label slot has content', async () => {
    const { tag } = uniqueSubclass();
    const el = document.createElement(tag);
    el.innerHTML = '<span slot="label">Subscribe</span>';
    document.body.appendChild(el);
    await raf();
