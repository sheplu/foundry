import type { Meta, StoryObj } from '@storybook/web-components';
import { html } from 'lit';
import { FoundrySwitch } from './switch.ts';

FoundrySwitch.define();

interface SwitchArgs {
  label: string;
  checked: boolean;
  required: boolean;
  disabled: boolean;
  value: string;
}

const meta: Meta<SwitchArgs> = {
  title: 'Forms/Switch',
  component: 'foundry-switch',
  argTypes: {
    label: { control: 'text' },
    checked: { control: 'boolean' },
    required: { control: 'boolean' },
    disabled: { control: 'boolean' },
    value: { control: 'text' },
  },
  args: {
    label: 'Enable notifications',
    checked: false,
    required: false,
    disabled: false,
    value: 'on',
  },
};

export default meta;

type Story = StoryObj<SwitchArgs>;

export const Default: Story = {
  render: ({ label, checked, required, disabled, value }) => html`
    <foundry-switch
      value=${value}
      ?checked=${checked}
      ?required=${required}
      ?disabled=${disabled}
    >
      <span slot="label">${label}</span>
    </foundry-switch>
  `,
};

export const Checked: Story = {
  parameters: { controls: { disable: true } },
  render: () => html`
    <foundry-switch checked name="notifications" value="on">
      <span slot="label">Enable notifications</span>
    </foundry-switch>
  `,
};

export const Required: Story = {
  parameters: { controls: { disable: true } },
  render: () => html`
    <foundry-switch required name="consent" value="yes">
      <span slot="label">Accept terms <em>*</em></span>
    </foundry-switch>
  `,
};

export const Disabled: Story = {
  parameters: { controls: { disable: true } },
  render: () => html`
    <div style="display:flex; flex-direction:column; gap:0.75rem;">
      <foundry-switch disabled>
        <span slot="label">Disabled + off</span>
      </foundry-switch>
      <foundry-switch disabled checked>
        <span slot="label">Disabled + on</span>
      </foundry-switch>
    </div>
  `,
};

export const InForm: Story = {
  parameters: { controls: { disable: true } },
  render: () => html`
    <form
      style="display:flex; flex-direction:column; gap:0.75rem; max-width:320px; font-family:sans-serif;"
      @submit=${(e: Event) => {
        e.preventDefault();
        const form = e.currentTarget as HTMLFormElement;
        const data = new FormData(form);
        const record: Record<string, string> = {};
        for (const [k, v] of data.entries()) record[k] = String(v);
        const out = form.parentElement?.querySelector('pre');
        if (out) out.textContent = JSON.stringify(record, null, 2);
      }}
    >
      <foundry-switch name="notifications" value="on">
        <span slot="label">Enable notifications</span>
      </foundry-switch>
      <foundry-switch name="beta" value="yes" checked>
        <span slot="label">Join beta program</span>
      </foundry-switch>
      <button type="submit">Save preferences</button>
    </form>
    <pre
      style="margin-block-start:0.75rem; padding:0.5rem; background:var(--foundry-color-surface-subtle); border-radius:var(--foundry-radius-sm); font-family:ui-monospace, monospace; font-size:0.8125rem;"
    ></pre>
  `,
};

const panel = (theme: 'light' | 'dark') => html`
  <div
    data-theme=${theme}
    style="padding:1rem; background:var(--foundry-color-surface); color:var(--foundry-color-text-body); border-radius:var(--foundry-radius-md); font-family:sans-serif;"
  >
    <p style="margin:0 0 0.5rem;">${theme}</p>
    <div style="display:flex; flex-direction:column; gap:0.5rem;">
      <foundry-switch>
        <span slot="label">Off</span>
      </foundry-switch>
      <foundry-switch checked>
        <span slot="label">On</span>
      </foundry-switch>
      <foundry-switch required invalid>
        <span slot="label">Invalid</span>
      </foundry-switch>
      <foundry-switch disabled>
        <span slot="label">Disabled</span>
      </foundry-switch>
    </div>
  </div>
`;

export const Theming: Story = {
  parameters: { controls: { disable: true } },
  render: () => html`
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:2rem;">
      ${panel('light')}
      ${panel('dark')}
    </div>
  `,
};
